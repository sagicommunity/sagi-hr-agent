...
// 2026-08-18: 'Не подходит' добавлен вслед за тем же списком в api/pipeline.js — иначе кандидаты
// с этой стадией (отказ на гейт-вопросе hh.kz) выпадали из разбивки byStage в общий «Новый».
const STAGES = ['Новый', 'Ожидает ответа', 'Ответил', 'Приглашён', 'Обучение', 'Стажировка', 'Трудоустроен', 'На связи', 'Квалификация', 'Интервью', 'Оффер', 'Не подходит', 'Отказ', 'Ушёл'];
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 2026-08-18, по замечанию Sagi: в воронке не было видно, сколько людей явно отказались
    // (написали, что формат/холодные звонки не подходят) — раньше это число пряталось только
    // текстом в шапке (funnelMeta). Добавляем отдельной строкой в конец таблицы. Не встраиваем
    // её в основную последовательность выше (между «ответили на гейт» и «прошли гейт»), чтобы не
    // ломать расчёт fromPrevPct для шагов после неё — это боковая ветка (отказ), а не следующий
    // шаг воронки.
    hhFunnel.push({
      key: '↳ Не подходит (отказались на гейт-вопросе)',
      count: hhTotalDeclined,
      fromPrevPct: null,
      fromStartPct: Math.round((hhTotalDeclined / funnelFirst) * 1000) / 10,
    });
