...
export default async function handler(req, res) {
  ...
  try {
    ...
    // а сразу зовём на стажировку. Отсутствие опыта холодных звонков, компьютера или интернета —
    // НЕ причина для отказа (см. SCREEN_SYS/SCREEN_SYS_REMOTE выше), этому всему учат на
    // стажировке, компьютер понадобится не с первого дня. «Отказ» от ИИ-скринера теперь ставится
    // только за реальные красные флаги (грубость, явный отказ работать/учиться в принципе).
