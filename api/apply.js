...
ВАЖНО: это не холодные продажи — кандидат работает с уже существующими клиентами компании. Резюме не запрашиваем — оценивай СТРОГО по структурированным ответам анкеты ниже, не додумывай. Отсутствие опыта именно в клиентском сервисе/удержании — НЕ повод для отказа само по себе.

Реальные красные флаги, из-за которых можно поставить «Отказ»:
- кандидат прямо говорит, что общение с клиентами / работа на удержание — не его(её);
- явно нет компьютера/ноутбука или стабильного интернета — обязательное условие для полностью удалённой работы;
- на вопрос про совмещение прямо говорит, что не готов(а) менять приоритеты ради этой работы — неважно, что именно занимает время;
- полное отсутствие готовности пользоваться ИИ-инструментами в повседневной работе (вариант «не готов(а) учиться») — для этой роли это рабочий инструмент, не опция;
- явная грубость, неадекватность или полное отсутствие мотивации в комментарии.

Если кандидат отвечает, что что-то отнимает время, но готов(а) расставить приоритеты — это НЕ автоматический отказ, но упомяни в summary как момент для уточнения лично.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!name || !contact || !city || !source || !expSales || !techReady || !noCombine || !startWhen || (isSuccess && !aiComfort)) {
    ...
    if (isSuccess) answers.push({ q: 'Комфорт с ИИ-инструментами (ChatGPT/Claude) в работе', a: aiComfort });
    ...
    // 2026-08-23: для success_remote ЕЩЁ НЕТ готового процесса выхода (нет модулей обучения, как у
    // sales) — Sagi это явно не определил. Пока используем нейтральный текст «заявка получена, с
    // вами свяжутся», БЕЗ выдумывания шагов регистрации/обучения, которых не существует. Заменить,
    // как только Sagi опишет реальный следующий шаг для этой роли (см. список вопросов, отправленный
    // Sagi 2026-08-23).
    ...
    const inviteTextSuccess = `${name}, спасибо за отклик на позицию «${vacTitle}»! Заявку получили, ответы посмотрели — в ближайшее время с вами свяжутся по контакту, который вы указали, и расскажут о следующем шаге.\n\nЕсли появятся вопросы, пишите в WhatsApp: +7 707 700 0087.`;
    const inviteText = isSuccess ? inviteTextSuccess : inviteTextSales;
