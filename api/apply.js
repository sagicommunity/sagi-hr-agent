...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Кнопка «Перейти на обучение» под сообщением на apply.html — только для ветки продаж, только
    // когда реально приглашён (message === inviteTextSales): у success/support нет 20-модульного
    // трека обучения на hr.sagibonus.com (?start=1), кнопка туда была бы некуда вести. Ссылку на
    // чек-лист адаптации (onboarding.html, 2026-08-27) им всё равно даём — но текстом внутри
    // самого inviteText*, а не отдельной кнопкой (см. isSuccess/isSupport-специфичные тексты выше).
    const trainingCta = !isSuccess && !isSupport && invited;

    res.status(200).json({ ok: true, score: evaln.score, verdict: evaln.verdict, summary: evaln.summary, matchPercent: evaln.matchPercent, invited, message, trainingCta });
  } catch (e) {
