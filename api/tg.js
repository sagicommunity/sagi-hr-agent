...
const VAC = {
  ...
  success:        { key: 'success',        title: 'Success Manager (Сервис-менеджер по работе с партнёрами)', format: 'удалённо' },
};
...
Давайте начнём. Выберите вакансию, на которую откликаетесь (отправьте 1, 2, 3, 4 или 5):
...
5) Success Manager (сервис-менеджер по работе с партнёрами)
   - Полностью удалённо. График 5/2, 09:00-18:00
   - Оклад 200 000 ₸ + бонусы, средний доход 450 000-600 000 ₸
...
function parseVacancy(text) {
  ...
  const isSuccess = /success|сервис.?менедж|аккаунт.?менедж|account manager|customer success|работе с партн[её]р/.test(s);
...
function questions(vac) {
  ...
  if (vac === 'success') return { ...common, ...successQuestions() };
...
// ---- Вопросы для Success Manager (сервис-менеджер по работе с партнёрами) ----
function successQuestions() {
  const title = VAC.success.title;
  return {
    name: `Отлично, вакансия "${title}". Давайте начнём: как вас зовут? (имя и фамилия)`,
    phone: 'Приятно познакомиться! Оставьте ваш номер телефона / WhatsApp для связи.',
    exp: 'Расскажите про опыт работы с клиентами: сервис, поддержка, аккаунт-менеджмент или похожее. Где именно и сколько по времени?',
    languages: 'Насколько свободно вы говорите на казахском и русском языках? Оба языка нужны для общения с партнёрами.',
    pc_crm: 'Уверенно ли вы работаете на компьютере? Был ли опыт работы в CRM-системах?',
    schedule: 'Формат работы: полностью удалённо, график 5/2 с 09:00 до 18:00. Подходит ли вам такой график? Не планируете совмещать с другой постоянной работой?',
    start: 'И последнее: когда вы готовы приступить к работе?',
  };
}
...
const ORDER = {
  ...
  success: ['name', 'phone', 'exp', 'languages', 'pc_crm', 'schedule', 'start'],
};
...
const SCREEN_SUCCESS = `Ты — HR-скринер Sagi (loyalty-платформа для B2B МСБ). Оцениваешь кандидата на позицию Success Manager (сервис-менеджер по работе с партнёрами): постоянная коммуникация с клиентами-партнёрами (звонки, чаты, встречи), продление подписок, работа на удержание, обработка жалоб и запросов, проактивная работа на лояльность, онлайн-встречи/вебинары, ведение CRM. Работа полностью удалённая, график 5/2, 09:00-18:00.
ЧТО ВАЖНО: грамотная связная речь на русском и казахском языках, клиентоориентированность и стрессоустойчивость, уверенное владение ПК, готовность работать по фиксированному графику без совмещения с другой постоянной работой. Опыт в сервисе/поддержке/аккаунт-менеджменте — плюс, опыт в B2B продажах — тоже плюс, но не обязателен.
КРАСНЫЕ ФЛАГИ → «Отказ»/низкий балл: грубость или явно неграмотная речь, отсутствие казахского ИЛИ русского на приемлемом уровне, нежелание работать по графику 5/2 или совмещение с другой полной занятостью, явное отсутствие интереса к работе с клиентами.
Верни ТОЛЬКО валидный JSON без markdown:
{"score": <0-10>, "verdict": "Брать на интервью" | "Резерв" | "Отказ", "summary": "<2-3 предложения>"}`;
...
async function screen(vac, name, fullText) {
  ...
  const sys = vac === 'sales' ? SCREEN_SALES : vac === 'sales_remote' ? SCREEN_SALES_REMOTE : vac === 'success' ? SCREEN_SUCCESS : SCREEN_SUPPORT;
  try {
...
function answerFaq(text, vac) {
  ...
  if (/(зарплат|оклад|сколько плат|доход|ставк|процент|з\/п|\bзп\b|оплат|деньг|kpi|бонус|заработ)/.test(t)) {
    ...
    if (vac === 'success') return 'По Success Manager: оклад 200 000 ₸ + бонусы за результат. Средний совокупный доход 450 000-600 000 ₸. Детали на собеседовании.';
    return 'Зарплата зависит от вакансии, после выбора я подскажу. По продажам (офис и удалёнка): оклад + KPI + %; по техподдержке: 200 000 ₸ (офис) или 100 000 ₸ фикс (удалёнка, вечер) + бонусы; по Success Manager: 200 000 ₸ + бонусы.';
  }
  if (/(удал[её]н|офис|график|формат|режим|из дома|онлайн|город|где наход|локац|астан|где работа|смена|часы)/.test(t)) {
    ...
    if (vac === 'success') return 'Полностью удалённо, график 5/2 с 09:00 до 18:00.';
    return 'Формат зависит от вакансии: продажи и техподдержка-офис это офис в Астане, продажи-удалёнка, техподдержка-удалёнка и Success Manager из дома.';
  }
  if (/(обязанност|что делать буду|что за работ|чем занима.{0,4}буд|задач)/.test(t)) {
    ...
    if (vac === 'success') return 'Success Manager держит связь с клиентами-партнёрами: звонки, чаты, встречи, продление подписок, работа на удержание, обработка жалоб и запросов, повышение лояльности, онлайн-встречи и вебинары, ведение CRM.';
    return 'Зависит от вакансии: продажи (звонки, встречи/демо, сделки), техподдержка (обращения клиентов, заявки в CRM, консультации) или Success Manager (сопровождение и удержание клиентов-партнёров).';
  }
...
function buildFullText(vac, d) {
  ...
  if (vac === 'success') {
    return `Опыт работы с клиентами: ${d.exp || '—'}\nЯзыки (казахский/русский): ${d.languages || '—'}\nПК/CRM: ${d.pc_crm || '—'}\nГрафик 5/2 удалённо, без совмещения: ${d.schedule || '—'}\nГотовность выйти на работу: ${d.start || '—'}`;
  }
  ...
  try {
    ...
    if (st.step === 'vacancy') {
      ...
      if (!vac) {
        ...
        await tgSend(chat, (f ? f + '\n\n' : 'Не понял выбор. ') + 'Напишите 1, 2, 3, 4 или 5, на какую вакансию откликаетесь.\n\n' + MENU);
        res.status(200).json({ ok: true }); return;
      }
